...
int main(int argc, char **argv)
{
	...
	geometry_msgs::TransformStamped firstTransformTf;
	...
	while (ros::ok())
	{
		...
		try
		{
			...
			if (init)
			{
				firstTransformTf = transformTf;
				tf::transformMsgToEigen(transformTf.transform, firstTransform);
				init = false;
			}
		}
		...
		// transformTf.transform = transformTf.transform

		Eigen::Affine3d currentTransform;
		tf::transformMsgToEigen(transformTf.transform, currentTransform);
		currentTransform = firstTransform.inverse()*currentTransform;
		tf::transformEigenToMsg(currentTransform, transformTf.transform);
